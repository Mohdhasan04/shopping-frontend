          ...
          <div className="md:w-1/3 lg:w-1/4 relative overflow-hidden">
            <div className="relative h-56 md:h-full bg-gray-100">
              ...
              <img
                src={imageUrl}
                alt={product.name}
                className={`w-full h-full object-cover transition-all duration-700 group-hover:scale-110 ${imageLoaded && !imageError ? 'opacity-100' : 'opacity-0'
                  }`}
                onLoad={() => {
                  setImageLoaded(true);
                  setImageError(false);
              ...
              <button
                ...
                className={`absolute top-3 right-3 p-2.5 bg-white/90 backdrop-blur-sm rounded-full shadow-lg transition-all duration-300 ${wishlistStatus ? 'text-red-500' : 'text-gray-600 hover:text-red-500'
      ...
      <Link to={`/product/${product.id}`} className="block">
        ...
        <div className="relative h-64 overflow-hidden bg-gray-100">
          ...
          <img
            src={imageUrl}
            alt={product.name}
            className={`w-full h-full object-cover transition-all duration-700 ${imageLoaded && !imageError ? 'opacity-100' : 'opacity-0'
          ...
          <div className={`absolute top-3 right-3 flex flex-col gap-2 transition-all duration-300 ${isHovered ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-2'
            ...
            <button
              onClick={handleWishlistToggle}
              disabled={isWishlistLoading}
              className={`p-2.5 bg-white/90 backdrop-blur-sm rounded-full shadow-lg transition-all duration-300 ${wishlistStatus ? 'text-red-500 shadow-red-200' : 'text-gray-600 hover:text-red-500'
                } ${isWishlistLoading ? 'animate-pulse' : 'hover:scale-110'}`}
            >
              {isWishlistLoading ? (
                <FiClock className="w-4 h-4 animate-spin" />
          ...
          {/* Add to Cart Overlay */}
          <div className={`absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/60 via-black/30 to-transparent transition-all duration-500 ${isHovered ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            }`}>
            <button
              onClick={handleAddToCart}
              disabled={isOutOfStock}
